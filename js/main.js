...
document.addEventListener('DOMContentLoaded', function() {
  ...
  orgCards.forEach(function(card) {
    card.addEventListener('click', function() {
      var bioId = 'bio-' + this.getAttribute('data-bio');
      var bio = document.getElementById(bioId);
      if (bio) {
        var isOpen = bio.style.display === 'block';
        // Close all
        document.querySelectorAll('.org-bio').forEach(function(b) { b.style.display = 'none'; });
        document.querySelectorAll('.org-card').forEach(function(c) { c.classList.remove('open'); });
        // Toggle
        if (!isOpen) {
